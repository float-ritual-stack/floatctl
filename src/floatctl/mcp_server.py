...
async def get_prompt(
    ...
    """Get a prompt from Evan's library. 
    
    Search by name, tags, or keywords to find the right prompt for the situation.
    Examples: 'consciousness', 'ritual', 'neuroqueer', 'shack', 'context crunch'
    """
    ...
    if not results:
        return {
            "error": f"No prompts found for '{query}'",
            "available_prompts": list(PROMPT_LIBRARY.keys()),
            "suggestion": "Try: consciousness, ritual, neuroqueer, shack, or context"
        }
    ...
    # Update usage count
    PROMPT_LIBRARY[best_match["name"]]["usage_count"] += 1
    ...
    return {
        ...
        "other_matches": [r["name"] for r in results[1:3]]  # Show 2 other options
    }
